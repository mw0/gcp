...
from graphlab_util import lockfile as _lockfile
...
class Session(object):
    ...
    def __init__(self, location):
        ...
        if not _os.path.exists(version_file):
            ...
            with self._lock:
                try:
                    version = int(open(version_file).readlines()[0].strip())
                except IOError as e:
                    raise IOError(("Corruption in Graphlab-Create artifacts directory:"
                            " Missing version file. (Trace %s)" % e))
        ...
        if version == 2:
            ...
            # Warn the user.
            __LOGGER__.info(
             "Saved jobs and environments saved using GraphLab-Create v1.3 cannot be loaded using GraphLab-Create v1.4.")
            __LOGGER__.info(
        ...
        if not _os.path.exists(version_file) or version != _SESSION_VERSION:
            with self._lock:
                v_file_handle = open(version_file, 'w')
