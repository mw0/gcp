...
from _dml_cluster import DmlExecutionEngine as _dml_ee
...
def create(name, s3_path, ec2_config, num_hosts=1, additional_packages=[],
           idle_shutdown_timeout = 10 * 60):
    '''
    Create a Dato cluster in EC2.
    ...
    idle_shutdown_timeout : int, optional
        ...
        it will be shutdown. Default value is ten minutes. Set to a negative value,
    ...
    # Make sure we can save to S3 path, before bring up EC2 host(s)
    cluster._save()
...
class Ec2Cluster(_Environment):
    ...
    def _load_version(cls, unpickler, version):
        ...
        new = cls(obj.ec2_config, obj.s3_state_path, obj.name, obj.num_hosts, obj.additional_packages,
                  obj.idle_shutdown_timeout, session_aware = False)
    ...
    def __init__(self, ec2_config, s3_state_path, name, num_hosts, additional_packages,
                 idle_shutdown_timeout, session_aware=True):
       super(Ec2Cluster, self).__init__(name, session_aware)
        ...
        try:
            ...
            if not _silent:
                _log.warning('Exception trying to load updated configuration '
                             'from S3 path %s. Exception: %s' % (s3_state_path, e))
            return False
