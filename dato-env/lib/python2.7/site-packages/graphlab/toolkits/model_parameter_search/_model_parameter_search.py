...
from graphlab.deploy import environments as _environments
...
class _ModelSearchJob(object):
    """
    This model should not be constructed directly.  Instead, use
    :func:`graphlab.model_parameter_search.create`,
    :func:`graphlab.random_search.create`, :func:`graphlab.manual_search.create`,
    :func:`graphlab.grid_search.create`.
    """

    def __init__(self, factory, parameter_sets,
                 name,
                 strategy=None,
                 environment=None, return_model=True):
        """
        ...
        if metric is None:
            for m in chosen_metrics:
                if m in results.column_names():
                    metric = m
                    ascending = ('accuracy' in metric) or ('precision' in metric)
                    break
...
def _create_model_search(datasets,
    ...
    if perform_trial_run:
        ...
        # creating jobs.
        trial_run = _ModelSearchJob(_train_test_model,
                                    trial_params,
                                    name=job_name,
                                    strategy=strategy,
                                    environment=environment,
                                    return_model=return_model)
        trial_results = trial_run.get_results()  # blocking
    ...
    m = _ModelSearchJob(_train_test_model,
