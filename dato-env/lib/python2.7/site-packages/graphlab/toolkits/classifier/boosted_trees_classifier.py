...
class BoostedTreesClassifier(_Classifier):
    ...
    def extract_features(self, dataset):
        """
        For each example in the dataset, extract the leaf indices of each tree
        as features.

        For multiclass classification, each leaf index contains #num_class
        numbers.

        The returned feature vectors can be used as input to train another classifier
        such as a :py:class:`~graphlab.logistic_classifier.LogisticClassifier`,
        ...
        >>> data['tree_features'] = model.extract_features(data)
        """
        _mt._get_metric_tracker().track('toolkit.classifier.boosted_trees_classifier.extract_features')
        _raise_error_if_not_sframe(dataset, "dataset")
        options = dict()
        options.update({'model': self.__proxy__,
                        'model_name': self.__name__,
    ...
    def _get_queryable_methods(cls):
        ...
        return {'predict': {'dataset': 'sframe'},
                ...
                'extract_features': {'dataset': 'sframe'}}
