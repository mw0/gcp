...
class OneHotEncoder(Transformer):
    ...
    * **list** : Each value in the list is treated like an individual string.
                 Hence, a *list* of categorical variables can be used to
                 represent a feature where all categories in the list
                 are simultaneously `hot`.

    * **dict** : They key of the dictionary is treated as a `namespace` and the
                 value is treated as a `sub-category` in the `namespace`. The
                 categorical variable being encoded in this case is a combination
                 of the `namespace` and the `sub-category`.
